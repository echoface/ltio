...
bool IOBuffer::EnsureWritableSize(int32_t len) {
  if (data_.capacity() - write_index_ >= len) {
    return true;
  }
  ...
  if (CanWriteSize() + read_index_ >= len)  {
    ...
    LOG_IF(INFO, data_.capacity() > kWarningBufferSize) << "Buffer Size > 32M";
...
inline uint8_t* IOBuffer::GetRead() {
  return &data_[read_index_];
}
inline uint8_t* IOBuffer::GetWrite() {
  return &data_[write_index_];
}
inline int32_t IOBuffer::CanReadSize() {
...
void IOBuffer::WriteRawData(const uint8_t* data, int32_t len) {
