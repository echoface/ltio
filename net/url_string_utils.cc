...
std::string GetIP(const std::string scheme_ip_port) {
  int pos = scheme_ip_port.find(scheme_ip_separator);
  int port_pos = scheme_ip_port.find(ip_port_separator, pos + scheme_ip_separator.size());
  if (pos == std::string::npos || port_pos == std::string::npos) {
    return "";
  }
  int ip_start = pos + scheme_ip_separator.size();
...
uint16_t GetPort(const std::string scheme_ip_port) {
  ...
  int port_start = port_pos + ip_port_separator.size();
...
bool ParseSchemeIpPortString(const std::string uri, SchemeIpPort& out) {
  int pos = uri.find(scheme_ip_separator);
  int port_pos = uri.find(ip_port_separator, pos + scheme_ip_separator.size());
  if (pos == std::string::npos || port_pos == std::string::npos) {
    return false;
  }
