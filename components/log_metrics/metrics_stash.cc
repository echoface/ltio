...
StashQueueGroup::StashQueueGroup(uint32_t count) : queues_(count) {
  index_.store(0);
}
...
void MetricsStash::RegistDistArgs(const std::string& name,
                                  const MetricDistArgs arg) {
  if (running_) {
