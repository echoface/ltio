...
int FileLoader::Load() {
  ...
  for (auto& file : files_) {
    ifs.open(file);
    if (!ifs.is_open()) {
      LOG(ERROR) << __FUNCTION__ << " file [" << file
                 << "] can't be open correctly";
      continue;
    }

    {
      base::ScopedGuard guard([&]() { ifs.close(); });
...
bool FileLoader::ParseAndCheckConfig() {
  ...
  for (Json::const_iterator iter = resources.begin(); iter != resources.end();
       iter++) {
    CHECK((*iter).is_object());
    std::string file = iter->value("path", "");
    if (file.empty()) {
      LOG(INFO) << __FUNCTION__ << " resource:" << *iter
                << " be omited for empty path";
      continue;
    }
